...
exports.updateUser = async (req, res) => {
  var { username, email, roles } = req.body;
  try {
    ...
    if (!user) {
      ...
      //updating username if
      await models.users.update({ username }, { where: { email } });
      const rolesArray = await models.master_roles.findAll({
        attributes: ["id", "role_name"],
        where: { role_name: { [models.Sequelize.Op.in]: [...roles] } },
      });
      //Delete previous user_roles and update new ones
      await models.user_roles.destroy({ where: { user_id: user.id } });
      //Bulk Create User Roles with updated ones
    ...
    res.status(400).send({ message: error.message || "Some error occured!" });
  }
};
