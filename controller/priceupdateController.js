  ...
  try {
        ...
        if (products.length > processed_product_count) {
          ...
          if (product_obj) {
            if (product_obj.trans_sku_lists.length > 0) {
              ...
              writelog(
                "sku count ------- " + product_obj.trans_sku_lists.length
              );
              console.log(
                "sku count ------- " + product_obj.trans_sku_lists.length
              );
    ...
    async function processskus() {
                ...
                models.diamond_price_settings
                  ...
                  .then(async (diamondcharge) => {
                    if (diamondcharge) {
                      ...
                      var diamondmargin =
                        ((diamondsellingprice - diamondcost) / diamondcost) *
                        100;
                      var diamondprice = {
                        component:
                          "diamond" +
                        ...
                        material_name:
                          diamondobj.diamond_clarity +
                          "" +
                          diamondobj.diamond_colour,
                        id: uuidv1(),
      ...
      async function updateskuprice() {
        ...
        writelog(
          "gemstone_percentage value" + JSON.stringify(material_price_obj)
        );
        ...
        markupobj.forEach(async (markup) => {
          if (markup.material == "Gold") {
            goldmarkupvalue =
              goldsellingprice + goldsellingprice * (markup.markup_value / 100);
            var query =
        ...
        var golddiscount_percentage =
          golddiscountvalue / total_sku_discountvalue;
        ...
        var gemstone_percentage =
          gemstonediscountvalue / total_sku_discountvalue;
        ...
        writelog(
          "gemstone_percentage value" + discount_price_distribute_percentage
        );

        if (makingchargediscountvalue > 0) {
          writelog(
            "makingchargediscountvalue before apply gold" +
              makingchargediscountvalue
          );
          ...
          writelog(
            "makingchargediscountvalue value" + makingchargediscountvalue
          );
        }
        if (gemstonediscountvalue > 0) {
          writelog(
            "gemstonediscountvalue before apply gold" + gemstonediscountvalue
          );
          ...
          writelog(
            "gemstonediscountvalue after apply gold" + gemstonediscountvalue
          );
        }
        golddiscountvalue = goldmarkupvalue;
        if (diamonddiscountvalue > 0) {
          writelog(
            "diamonddiscountvalue before apply gold" + diamonddiscountvalue
          );
          ...
          writelog(
            "diamonddiscountvalue after apply gold" + diamonddiscountvalue
          );
        }
        ...
        var mkquery =
          "UPDATE pricing_sku_metals SET discount_price = " +
          makingchargediscountvalue +
          " where product_sku ='" +
          productskus[skucount].generated_sku +
          "' and material_name = 'makingcharge'";
...
exports.silverPriceUpload = (req, res) => {
  try {
    let updateSkuPrice = (trans_sku) => {
      return new Promise((resolve, reject) => {
        let { product_id } = trans_sku;
        delete trans_sku.product_id;
        models.trans_sku_lists
          .update(
            {
              ...trans_sku,
    ...
    let { sku_details } = req.body;
    if (!Array.isArray(sku_details) && sku_details.length == 0) {
      return res
        .status(400)
    ...
    Promise.all(sku_details.map(updateSkuPrice))
      ...
      where c.id = sub.shopping_cart_id`);
        res.status(200).send({ message: "Silver Price Updated!" });
      })
      .catch((err) => {
