  ...
  try {
    ...
    function update_product_images(product_images) {
      ...
      models.product_images
        .bulkCreate(product_images_arr, { individualHooks: true })
    ...
    function updateproduct() {
      ...
      var product_model_obj = {
        ...
        default_weight:
          product_obj[processed_product_count].weight_for_default_size,
        ...
        product_vendor_code:
          product_obj[processed_product_count].vendor_Product_Code,
      ...
      materials_arr.forEach(async (metal) => {
        let response = await models.product_materials.findOne({
          where: {
            ...
            product_sku: metal["product_gemstones"],
          },
        });
        if (!response) {
          var metalobj = {
    ...
    function update_product_gemstones(gemstoneobj) {
      ...
      models.product_gemstones
        .bulkCreate(product_gemstones, { individualHooks: true })
        .then(function (response) {
  ...
  try {
    ...
    if (isfirstime) {
      ...
      let products_list = await models.trans_sku_lists.findAll({
        attributes: [[sequelize.literal("DISTINCT(product_id)"), "products"]],
        where: {
          updatedAt: {
